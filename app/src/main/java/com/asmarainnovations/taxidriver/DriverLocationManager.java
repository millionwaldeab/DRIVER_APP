...
public class DriverLocationManager {
	...
	private final GpsStatus mGpsStatus; // was = new GpsStatus(); and I changed
										// it
	...
	*//**
	...
	private class ListenerTransport extends ILocationListener.Stub {
		...
		ListenerTransport(MyLocationListener listener, Looper looper) {
			mListener = listener;
	...
	*//**
	 ...
	 *//*
	public MyLocationManager(ILocationManager service) {
		if (false) {
	...
	private DummyLocationProvider createProvider(String name, Bundle info) {
	...
	*//**
	 ...
	 *//*
	public DummyLocationProvider getProvider(String name) {
		if (name == null) {
	...
	*//**
	...
	public List<String> getProviders(Criteria criteria, boolean enabledOnly) {
		...
		for (String providerName : providers) {
			DummyLocationProvider provider = getProvider(providerName);
	...
	*//**
	...
	private LocationProvider best(List<String> providerNames) {
		List<LocationProvider> providers = new ArrayList(providerNames.size());
		...
		List<LocationProvider> tmp = new ArrayList();
		idx = 0;
		size = providers.size();
		while ((idx < size)
				&& (providers.get(idx).getPowerRequirement() == power)) {
		...
		List<LocationProvider> tmp2 = new ArrayList();
		idx = 0;
		size = tmp.size();
	...
	*//**
	...
	private void _requestLocationUpdates(String provider, long minTime,
			float minDistance, LocationListener listener, Looper looper) {
		if (minTime < 0L) {
			minTime = 0L;
		}
	...
	*//**
	 ...
	 *            the minimum distance interval for notifications, in meters
	 * @param intent
	 *            a {#link PendingIntet} to be sent for each location update
	 * 
	 * @throws IllegalArgumentException
	 *             if provider is null or doesn't exist
	 * @throws IllegalArgumentException
	...
	private void _requestLocationUpdates(String provider, long minTime,
		...
		try {
			mService.requestLocationUpdatesPI(provider, minTime, minDistance,
					intent);
		} catch (RemoteException ex) {
	...
	public void addProximityAlert(double latitude, double longitude,
		...
		if (false) {
			Log.d(TAG, "addProximityAlert: latitude = " + latitude
					...
					+ ", expiration = " + expiration + ", intent = " + intent);
		}
		try {
	...
	*//**
	 ...
	 *             Settings.Secure.ALLOW_MOCK_LOCATION} system setting is not
	 *             enabled
	 * @throws IllegalArgumentException
	...
	public void addTestProvider(String name, boolean requiresNetwork,
		...
		try {
			mService.addTestProvider(name, requiresNetwork, requiresSatellite,
	...
	*//**
	 * Sets a mock location for the given provider. This location will be used
	 * in place of any actual location from the provider.
	 * 
	 * @param provider
	 *            the provider name
	 * @param loc
	...
	*//**
	 * Sets a mock enabled value for the given provider. This value will be used
	 * in place of any actual value from the provider.
	 * 
	 * @param provider
	 *            the provider name
	 * @param enabled
	...
	*//**
	 * Sets mock status values for the given provider. These values will be used
	 * in place of any actual values from the provider.
	 * 
	 * @param provider
	 *            the provider name
	 * @param status
				...
				if (msg.what == NMEA_RECEIVED) {
					synchronized (mNmeaBuffer) {
						...
						for (int i = 0; i < length; i++) {
							...
							mNmeaListener.onNmeaReceived(nmea.mTimestamp,
									nmea.mNmea);
						}
						mNmeaBuffer.clear();
					}
				} else {
					// synchronize on mGpsStatus to ensure the data is copied
					// atomically.
	...
	*//**
	...
	public boolean addGpsStatusListener(GpsStatus.Listener listener) {
		...
		try {
			GpsStatusListenerTransport transport = new GpsStatusListenerTransport(
					listener);
	...
	*//**
	...
	public void removeGpsStatusListener(GpsStatus.Listener listener) {
		try {
			GpsStatusListenerTransport transport = mGpsStatusListeners
					.remove(listener);
			if (transport != null) {
	...
	*//**
	...
	public void removeNmeaListener(GpsStatus.NmeaListener listener) {
		try {
			GpsStatusListenerTransport transport = mNmeaListeners
					.remove(listener);
			if (transport != null) {
				mService.removeGpsStatusListener(transport);
			}
	...
	*//**
	 * Sends additional commands to a location provider. Can be used to support
	 * provider specific extensions to the Location Manager API
	 * 
	 * @param provider
	 ...
	 * @param extras
	 *            optional arguments for the command (or null). The provider may
	 *            optionally fill the extras Bundle with results from the
	 *            command.
	 * 
	...
	*//**
	 * Used by NetInitiatedActivity to report user response for network
	 * initiated GPS fix requests.
	 * 
	 * {@hide}
	 *//*
